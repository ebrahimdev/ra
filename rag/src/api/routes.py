...
async def search(req: SearchRequest):
    ...
    try:
        ...
        results = vector_store.search(semantic_query, req.k)
...
async def count_chunks():
    ...
    try:
        ...
        return {"count": stats.get("total_documents", 0)}
    except Exception as e:
...
async def clean_chunks():
    """Clean chunks by removing those that are too short."""
    try:
        deleted_count = vector_store.clean_chunks()
        return CleanChunksResponse(
            success=True,
            chunks_deleted=deleted_count,
            message=f"Deleted {deleted_count} short chunks"
        )
    except Exception as e:
